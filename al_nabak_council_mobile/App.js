...
const logo = require("./assets/alnabk-symbol.png");
...
export default function App() {
  ...
  const [apiBaseUrl, setApiBaseUrl] = useState(cleanBaseUrl(DEFAULT_API_BASE_URL));
  const [apiDraft, setApiDraft] = useState(cleanBaseUrl(DEFAULT_API_BASE_URL));
  ...
  const handleAuthStatus = useCallback((authenticated) => {
    ...
    <SafeAreaProvider>
      ...
      <View style={styles.appFrame}>
        <Header
          ...
          dir={dir}
        />
        <AuthProbe locale={locale} apiBaseUrl={apiBaseUrl} onAuthChange={(authenticated) => setIsLoggedIn(Boolean(authenticated))} />
            ...
            >
              ...
              {activeTab === "more" ? (
                <MoreScreen
                  ...
                  openWeb={openInAppWeb}
                  saveApi={() => {
                    setApiBaseUrl(cleanBaseUrl(apiDraft));
                    setNotice("");
                  }}
                  dir={dir}
                />
    ...
    <View style={styles.header}>
      <View style={[styles.headerTop, dir.row]}>
        ...
        <Pressable
          ...
          accessibilityLabel={locale === "en" ? "Switch to Arabic" : "Switch to English"}
          hitSlop={10}
          style={styles.langButton}
          onPress={() => setLocale(locale === "en" ? "ar" : "en")}
        >
          <Text style={styles.langButtonText} allowFontScaling={false}>{locale === "en" ? "AR" : "EN"}</Text>
        </Pressable>
      </View>
    </View>
  );
...
function OptionsSheet({ visible, locale, dir, close, selectTab, openLogin, openLogout, isLoggedIn }) {
  const options = [
    ...
    { key: "connection", title: t(locale, "connection"), body: t(locale, "connectionHelp"), action: () => selectTab("more") }
  ];
...
const webChromeScript = `
  (function () {
    ...
    function applyMobileChrome() {
      ...
      var existing = document.getElementById('alnabk-mobile-webview-style');
      if (!existing) {
        var style = document.createElement('style');
        style.id = 'alnabk-mobile-webview-style';
        style.textContent = [
...
const logoutScript = `
  (function () {
    setTimeout(function () {
      if (window.__alnabkLogoutStarted) return;
      var form = document.querySelector('form.nav-inline-form[action*="/logout"]');
      if (form) {
        window.__alnabkLogoutStarted = true;
        fetch(form.action, {
          method: 'POST',
          ...
          .catch(function (error) {
            window.__alnabkLogoutStarted = false;
            if (window.ReactNativeWebView) {
              window.ReactNativeWebView.postMessage(JSON.stringify({
                type: 'logoutResult',
                ok: false,
  ...
  const handleProbeMessage = (event) => {
    try {
      ...
      if (payload.type === "authStatus") {
        onAuthChange(Boolean(payload.authenticated));
      }
    } catch {
...
function MoreScreen({ locale, sites, apiBaseUrl, apiDraft, setApiDraft, openWeb, saveApi, dir }) {
  return (
    <View style={styles.section}>
      <Text style={[styles.screenTitle, dir.text]}>{t(locale, "more")}</Text>
      <UsefulSites locale={locale} sites={sites} dir={dir} openWeb={openWeb} />
      <View style={styles.card}>
        <Text style={[styles.cardTitle, dir.text]}>{t(locale, "connection")}</Text>
        <Text style={[styles.muted, dir.text]}>{t(locale, "apiHelp")}</Text>
        <Text style={[styles.smallText, dir.text]}>{t(locale, "connectedTo")}: {apiBaseUrl}</Text>
        <Input locale={locale} value={apiDraft} onChangeText={setApiDraft} placeholder={t(locale, "apiBaseUrl")} autoCapitalize="none" />
        <Pressable style={styles.primaryButton} onPress={saveApi}>
