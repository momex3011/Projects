...
app.config["BABEL_SUPPORTED_LANGUAGES"] = ["en", "ar"]
...
DEFAULT_SITE_LINK_DESCRIPTIONS = {
    "instagram_description_en": "Photos, announcements, and community updates from the council.",
    "instagram_description_ar": "صور وإعلانات وتحديثات مجتمعية من المجلس.",
    "whatsapp_description_en": "Quick contact and direct community updates through WhatsApp.",
    "whatsapp_description_ar": "تواصل سريع وتحديثات مباشرة للمجتمع عبر واتساب.",
    "facebook_description_en": "Public posts, news, and resident-facing updates.",
    "facebook_description_ar": "منشورات عامة وأخبار وتحديثات موجهة للسكان.",
    "twitter_description_en": "Short news updates, notices, and public announcements.",
    "twitter_description_ar": "تحديثات قصيرة وتنبيهات وإعلانات عامة.",
    "telegram_description_en": "Broadcast messages, alerts, and public channel updates.",
    "telegram_description_ar": "رسائل بث وتنبيهات وتحديثات القناة العامة.",
}
...
SOCIAL_LINK_PLATFORMS = [
    {
        ...
        "placeholder": "https://www.instagram.com/alnabk",
    },
    {
        "key": "whatsapp",
        "title": "WhatsApp",
    ...
    },
    {
        ...
        "placeholder": "https://www.facebook.com/alnabk",
    },
    {
        "key": "twitter",
        "title": "Twitter/X",
        ...
        "placeholder": "https://x.com/alnabk",
    },
    {
        "key": "telegram",
        "title": "Telegram",
...
class SiteLinksForm(FlaskForm):
    ...
    instagram_description_ar = TextAreaField(_l("Arabic description"), validators=[Optional(), Length(max=500)])
    ...
    whatsapp_description_ar = TextAreaField(_l("Arabic description"), validators=[Optional(), Length(max=500)])
    ...
    facebook_description_ar = TextAreaField(_l("Arabic description"), validators=[Optional(), Length(max=500)])
    ...
    twitter_description_ar = TextAreaField(_l("Arabic description"), validators=[Optional(), Length(max=500)])
    ...
    telegram_description_ar = TextAreaField(_l("Arabic description"), validators=[Optional(), Length(max=500)])
...
class DonationCampaignForm(FlaskForm):
    ...
    currency = SelectField(
        _l("Currency"),
        choices=[("USD", "USD"), ("SYP", "SYP"), ("EUR", "EUR"), ("TRY", "TRY")],
        default="USD",
        validators=[DataRequired()],
    )
    status = SelectField(
...
admin = Admin(app, name="CouncilTrack Admin", template_mode="bootstrap4", index_view=SecureAdminIndexView())
...
def bootstrap_database():
    ...
    if not User.query.filter_by(role=Role.ADMIN).first():
        admin_password = os.environ.get("DEFAULT_ADMIN_PASSWORD") or secrets.token_urlsafe(14)
        credential_path = os.path.join(BASE_DIR, "instance", "default_admin_credentials.txt")
        ...
        if not os.environ.get("DEFAULT_ADMIN_PASSWORD"):
            with open(credential_path, "w", encoding="utf-8") as credential_file:
                credential_file.write("username=admin\n")
                credential_file.write(f"password={admin_password}\n")
    if not DonationCampaign.query.first():
        db.session.add(
            DonationCampaign(
                title="Community Support Fund",
                description="A public fund for urgent community needs, families under pressure, and transparent civic support.",
                beneficiary="Al-Nabk residents",
                goal_amount=Decimal("10000.00"),
                currency="USD",
                payment_instructions="After submitting your donation, use the reference number in your bank transfer note or share it with the council office for confirmation.",
                status="Active",
            )
        )
        db.session.commit()
